    ...
    return (
      <div className="space-y-6 animate-in fade-in duration-500 w-full overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 w-full">
          ...
          <Card className="p-4 border-l-4 border-l-emerald-500 flex justify-between items-center">
             <div><p className="text-xs text-slate-500 font-bold uppercase tracking-wider">Clientes SAP</p><h3 className="text-2xl font-bold text-slate-900 mt-1">{clients}</h3></div>
             <div className="bg-emerald-50 p-3 rounded-xl text-emerald-600"><CheckCircle2 size={24}/></div>
          </Card>
          <Card className="p-4 border-l-4 border-l-indigo-500 flex justify-between items-center">
             <div><p className="text-xs text-slate-500 font-bold uppercase tracking-wider">Prospectos</p><h3 className="text-2xl font-bold text-slate-900 mt-1">{leads}</h3></div>
             <div className="bg-indigo-50 p-3 rounded-xl text-indigo-600"><Target size={24}/></div>
          </Card>
          <Card className={`p-4 border-l-4 flex justify-between items-center ${pending > 0 ? 'border-l-red-500 bg-red-50/30' : 'border-l-slate-300'}`}>
             <div><p className="text-xs text-slate-500 font-bold uppercase tracking-wider">Acciones Hoy</p><h3 className={`text-2xl font-bold mt-1 ${pending > 0 ? 'text-red-600' : 'text-slate-900'}`}>{pending}</h3></div>
          ...
          <Card className="p-4 md:p-6 h-full">
            ...
            <div className="space-y-3">
               {contacts.filter(c => c.next_action_date).sort((a,b) => new Date(a.next_action_date).getTime() - new Date(b.next_action_date).getTime()).slice(0,5).map(c => (
                 <div key={c.id} className="flex items-center justify-between p-3 bg-white rounded-xl border border-slate-100 shadow-sm active:bg-blue-50 transition-colors cursor-pointer group" onClick={() => { setEditingContact(c); setView('form'); }}>
                   <div className="min-w-0">
                     <span className="font-bold text-slate-800 text-sm block truncate">{c.next_action}</span>
                     <p className="text-xs text-slate-500 mt-1 flex items-center gap-1 truncate"><FileText size={12}/> {c.fiscal_name}</p>
                   </div>
               ...
               {contacts.length === 0 && <div className="p-6 text-center text-slate-400 border-2 border-dashed border-slate-100 rounded-xl">No hay acciones pendientes.</div>}
            </div>
          </Card>
      ...
      <div className="space-y-4 animate-in fade-in duration-500 pb-20 w-full overflow-hidden">
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
           ...
           <div className="relative w-full md:w-80">
             ...
             <input type="text" placeholder="Buscar..." className="w-full pl-10 pr-4 py-3 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition-all text-sm" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
           </div>
        </div>
                ...
                <div className="flex justify-between items-start gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-3 mb-2">
                      <h3 className="font-bold text-base md:text-lg text-slate-900 truncate">{c.fiscal_name}</h3>
                      <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full border w-fit ${c.sap_status === 'Cliente SAP' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-blue-50 text-blue-700 border-blue-200'}`}>{c.sap_status}</span>
                    </div>
        ...
        <div className="flex flex-col md:flex-row justify-between mb-4 sticky top-0 bg-slate-100/95 z-30 p-4 -mx-2 md:-mx-4 backdrop-blur-md border-b border-slate-200">
           <div className="mb-3 md:mb-0">
             <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
               {editingContact ? <Edit className="text-blue-600 w-5 h-5"/> : <UserPlus className="text-blue-600 w-5 h-5"/>}
               {editingContact ? 'Editar' : 'Nuevo Diagnóstico'}
             </h2>
           </div>
           <div className="flex gap-2 w-full md:w-auto">
             <Button variant="secondary" onClick={() => setView('list')} className="flex-1 md:flex-none justify-center">Cancelar</Button>
             <Button variant="primary" onClick={handleSubmit} icon={Save} disabled={saving} className="flex-1 md:flex-none justify-center">{saving ? '...' : 'Guardar'}</Button>
           </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 w-full px-1">
            {activeTab === 'sap' && (
                <Card className="p-4 md:p-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div><label className={labelClass}>Estado en SAP</label><select className={selectClass} value={formData.sap_status} onChange={e => handleChange('sap_status', e.target.value)}><option>Nuevo Prospecto</option><option>Lead SAP</option><option>Cliente SAP</option></select></div>
                        <div><label className={labelClass}>Código SAP</label><input className={inputClass} placeholder="Ej: C000450" value={formData.sap_id} onChange={e => handleChange('sap_id', e.target.value)} /></div>
                    </div>
                    <div className="flex justify-end mt-6 pt-4 border-t"><Button onClick={goToNextTab} icon={ArrowRight} variant="secondary" className="w-full md:w-auto">Siguiente</Button></div>
                </Card>
            )}

            {activeTab === 'registro' && (
                <Card className="p-4 md:p-8">
                    <SectionHeader title="Datos de Registro" icon={Briefcase} />
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div><label className={labelClass}>Nombre Fiscal *</label><input required className={inputClass} value={formData.fiscal_name} onChange={e => handleChange('fiscal_name', e.target.value)} /></div>
            ...
            {activeTab === 'negocio' && (
                <Card className="p-4 md:p-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div><label className={labelClass}>Productos</label><input className={inputClass} value={formData.main_products} onChange={e => handleChange('main_products', e.target.value)} /></div>
                    </div>
                    <div className="flex gap-3 mt-6 pt-4 border-t"><Button onClick={goToPrevTab} icon={ArrowLeft} variant="ghost" className="flex-1">Anterior</Button><Button onClick={goToNextTab} icon={ArrowRight} variant="secondary" className="flex-1">Siguiente</Button></div>
                </Card>
            )}
            ...
            {activeTab === 'materiales' && (
                <Card className="p-4 md:p-8 bg-slate-50">
                    <SectionHeader title="Materiales" icon={Package} />
                    {[1,2,3,4,5,6,7].map(num => {
                        if (!(num === 1 || formData[`hasMat${num}`])) return null;
                        return (
                            <div key={num} className="rounded-xl border border-slate-200 mb-6 bg-white overflow-hidden">
                                <div className="p-3 border-b bg-slate-50 flex justify-between items-center">
                                    <span className="text-xs font-bold px-2 py-1 rounded bg-blue-100 text-blue-700">MAT {num}</span>
                                    {num > 1 && <button type="button" onClick={() => handleChange(`hasMat${num}`, false)} className="text-slate-400"><X size={18}/></button>}
                                </div>
                                <div className="p-4 grid grid-cols-1 gap-4">
                                    <div><label className={labelClass}>Tipo</label><select className={selectClass} value={formData[`mat${num}_type`]} onChange={e => handleChange(`mat${num}_type`, e.target.value)}><option value="">Seleccionar...</option>{MATERIAL_OPTIONS.map(m => <option key={m} value={m}>{m}</option>)}</select></div>
                                    <div><label className={labelClass}>ID / Medidas</label><input className={inputClass} value={formData[`mat${num}_id`]} onChange={e => handleChange(`mat${num}_id`, e.target.value)} /></div>
                                    <div className="grid grid-cols-2 gap-3">
                                        <div><label className={labelClass}>Consumo</label><input className={inputClass} value={formData[`mat${num}_consumption`]} onChange={e => handleChange(`mat${num}_consumption`, e.target.value)} /></div>
                                        <div><label className={labelClass}>Precio</label><input className={inputClass} value={formData[`mat${num}_price`]} onChange={e => handleChange(`mat${num}_price`, e.target.value)} /></div>
                                    </div>
                                    <div><label className={labelClass}>Proveedor</label><input className={inputClass} value={formData[`mat${num}_supplier`]} onChange={e => handleChange(`mat${num}_supplier`, e.target.value)} /></div>
                                    <div><label className={labelClass}>Notas</label><input className={inputClass} value={formData[`mat${num}_notes`]} onChange={e => handleChange(`mat${num}_notes`, e.target.value)} /></div>
                                </div>
                            </div>
                        );
                    })}
                    {[1,2,3,4,5,6].map(num => {
                        if ((num === 1 || formData[`hasMat${num}`]) && !formData[`hasMat${num+1}`]) {
                            return <button key={num} type="button" onClick={() => handleChange(`hasMat${num+1}`, true)} className="w-full py-3 border-2 border-dashed border-blue-200 text-blue-600 rounded-lg font-bold mb-4 text-sm">+ Añadir Material</button>
                        }
                        return null;
                    })}
                    <div className="flex gap-3 mt-4 pt-4 border-t"><Button onClick={goToPrevTab} icon={ArrowLeft} variant="ghost" className="flex-1">Anterior</Button><Button onClick={goToNextTab} icon={ArrowRight} variant="secondary" className="flex-1">Siguiente</Button></div>
                </Card>
            )}

            {activeTab === 'maquinaria' && (
                <Card className="p-4 md:p-8">
                    <SectionHeader title="Maquinaria" icon={Factory} />
                    <div className="grid grid-cols-1 gap-6 mb-6">
                        <div className="p-4 bg-purple-50 rounded-xl border border-purple-100">
                            <label className="block text-xs font-bold text-purple-900 mb-3 uppercase">Calidad Percibida</label>
                            <div className="flex justify-between">{[1,2,3,4,5].map(v => (<button key={v} type="button" onClick={() => handleChange('quality_rating', v.toString())} className={`w-10 h-10 rounded-full font-bold shadow-sm ${formData.quality_rating === v.toString() ? 'bg-purple-600 text-white' : 'bg-white text-slate-600 border'}`}>{v}</button>))}</div>
                        </div>
                        <div className="p-4 bg-emerald-50 rounded-xl border border-emerald-100">
                            <label className="block text-xs font-bold text-emerald-900 mb-2 uppercase">¿Interés Sostenible?</label>
                            <select className="w-full p-2 border rounded bg-white" value={formData.sustainable_interest} onChange={e => handleChange('sustainable_interest', e.target.value)}><option>No</option><option>Sí</option></select>
                        </div>
                    </div>
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                        <h4 className="font-bold text-slate-800 mb-4 text-sm">Máquina Principal</h4>
                        <div className="grid grid-cols-1 gap-4">
                            <div><label className={labelClass}>Tipo</label><input className={inputClass} value={formData.mac1_type} onChange={e => handleChange('mac1_type', e.target.value)} /></div>
                            <div><label className={labelClass}>Marca</label><input className={inputClass} value={formData.mac1_brand} onChange={e => handleChange('mac1_brand', e.target.value)} /></div>
                            <div className="grid grid-cols-2 gap-3">
                                <div><label className={labelClass}>Edad</label><select className={selectClass} value={formData.mac1_age} onChange={e => handleChange('mac1_age', e.target.value)}><option>Nueva</option><option>Media</option><option>Antigua</option></select></div>
                                <div><label className={labelClass}>Estado</label><select className={selectClass} value={formData.mac1_status} onChange={e => handleChange('mac1_status', e.target.value)}><option>Ok</option><option>Averías</option><option>Cambio</option></select></div>
                            </div>
                        </div>
                    </div>
                    <div className="flex gap-3 mt-6 pt-4 border-t"><Button onClick={goToPrevTab} icon={ArrowLeft} variant="ghost" className="flex-1">Anterior</Button><Button onClick={goToNextTab} icon={ArrowRight} variant="secondary" className="flex-1">Siguiente</Button></div>
            ...
            {activeTab === 'necesidades' && (
                <Card className="p-4 md:p-8 border-l-4 border-l-blue-600">
                    <SectionHeader title="Necesidades" icon={Search} />
                    <div className="grid grid-cols-1 gap-6">
                        <div>
                            <label className="block text-sm font-bold text-slate-700 mb-3">Puntos de Dolor</label>
                            <div className="space-y-2">
                                {['Ahorro de costes', 'Renovación maquinaria', 'Mejorar estabilidad', 'Servicio Técnico', 'Reducir plástico'].map(opt => (
                                    <label key={opt} className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg border border-slate-200">
                                        <input type="checkbox" checked={formData.pain_points?.includes(opt)} onChange={() => handleMultiSelect('pain_points', opt)} className="w-5 h-5 text-blue-600 rounded"/> 
                                        <span className="text-sm font-medium text-slate-700">{opt}</span>
                                    </label>
                                ))}
                            </div>
                        </div>
                        <div><label className={labelClass}>Presupuesto</label><select className={selectClass} value={formData.budget} onChange={e => handleChange('budget', e.target.value)}><option>Sin presupuesto fijo</option><option>Partida anual</option><option>Solo precio bajo</option></select></div>
                    </div>
                    <div className="flex gap-3 mt-6 pt-4 border-t"><Button onClick={goToPrevTab} icon={ArrowLeft} variant="ghost" className="flex-1">Anterior</Button><Button onClick={goToNextTab} icon={ArrowRight} variant="secondary" className="flex-1">Siguiente</Button></div>
                </Card>
            )}
            ...
            {activeTab === 'cierre' && (
                <Card className="p-4 md:p-8 border-l-4 border-l-red-500 bg-red-50/10">
                    <SectionHeader title="Cierre" icon={Calendar} />
                    <div className="grid grid-cols-1 gap-6 mb-6">
                        <div>
                    ...
                    <div className="bg-white p-4 rounded-xl border border-red-200 shadow-sm">
                        <div className="grid grid-cols-1 gap-4">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <div><label className="block text-xs font-bold text-red-700 uppercase mb-1">HORA</label><input type="time" required className="w-full p-3 border border-red-200 rounded-lg" value={formData.next_action_time} onChange={e => handleChange('next_action_time', e.target.value)} /></div>
                            </div>
                            <div><label className="block text-xs font-bold text-red-700 uppercase mb-1">RESPONSABLE</label><input className="w-full p-3 border border-red-200 rounded-lg" value={formData.responsible} onChange={e => handleChange('responsible', e.target.value)} /></div>
                        </div>
                    </div>
                    <div className="flex flex-col-reverse gap-3 mt-6 pt-4 border-t border-red-200">
                        <Button onClick={goToPrevTab} icon={ArrowLeft} variant="ghost" className="w-full">Anterior</Button>
                        <Button variant="primary" type="submit" icon={CheckCircle2} className="w-full py-4 text-lg bg-gradient-to-r from-red-600 to-red-700 border-none shadow-xl">FINALIZAR</Button>
                    </div>
                </Card>
            )}
        </form>
